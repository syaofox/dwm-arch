...
{
  home.file = {
    ".local/share/dwm/dwm-session.sh" = {
      executable = true;
      text = ''
        #!${pkgs.bash}/bin/bash
        set -Eeuo pipefail
        ...
        xss-lock -- ${pkgs.slock}/bin/slock -m "Single is simple, double is double." &
        ...
        if [ ! -f "$HOME/.config/theme" ] && command -v generate-app-themes &>/dev/null && [ -d "$HOME/.config/theme-templates" ] && [ -n "$THEME" ] && [ -f "$HOME/.Xresources.d/''${THEME}" ]; then
            generate-app-themes "$HOME/.Xresources.d/''${THEME}"
        fi
