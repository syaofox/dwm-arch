...
stdenv.mkDerivation {
  ...
  installPhase = ''
    mkdir -p $out/bin
    cat > $out/bin/generate-app-themes << 'PYEOF'
#!/usr/bin/env python3
import sys, os, re
...
def hex_to_rgb(hex_color):
    ...
    return '{}, {}, {}'.format(int(hex_color[0:2], 16), int(hex_color[2:4], 16), int(hex_color[4:6], 16))
...
def hex_to_argb(hex_color):
    return '#ff{}'.format(hex_color.strip().lstrip('#'))

def hex_to_argb_50(hex_color):
    return '#80{}'.format(hex_color.strip().lstrip('#'))
...
def main():
    ...
    for tpl_name, rel_output in TEMPLATE_OUTPUTS:
        if rel_output.endswith('/gtk.css'):
            ...
            continue
        elif 'qt5ct-colors' in tpl_name and 'dark' not in tpl_name:
...
if __name__ == '__main__':
    main()
PYEOF
    chmod +x $out/bin/generate-app-themes
  '';

  meta = with lib; {
    description = "Generate app theme configs from Xresources";
