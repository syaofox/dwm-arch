...
GTK_THEME=$(grep -m1 '^#define\s\+GTK_THEME\s\+' "$THEME_FILE" 2>/dev/null | awk '{print $3}') || GTK_THEME="Mint-Y-Purple"
GTK_ICON=$(grep -m1 '^#define\s\+GTK_ICON\s\+' "$THEME_FILE" 2>/dev/null | awk '{print $3}') || GTK_ICON="Mint-Y-Purple"
GTK_FONT=$(grep -m1 '^#define\s\+GTK_FONT\s\+' "$THEME_FILE" 2>/dev/null | awk '{print $3}') || GTK_FONT="Noto Sans 11"
